...
const Fluege = () => {
  ...
  useEffect(() => {
    const fetchFluegeAndReviews = async () => {
      try {
        // Fetch Flights
        const flightResponse = await axios.get('http://localhost:3001/');
        setFluege(flightResponse.data);

        // Fetch Reviews
        const reviewResponse = await axios.get('http://localhost:3003/admin');
        const reviewsByFlug = reviewResponse.data.reduce((acc, review) => {
        ...
        setReviews(reviewsByFlug);
        setLoading(false);

        // Preload images
        const imagesToLoad = [];
        for (let i = 1; i <= 15; i++) {
          const imagePath = `/Plane${i}.jpg`;
        ...
        console.error('Error fetching flights and reviews:', error);
        setLoading(false);
      }
    };

    fetchFluegeAndReviews();
  }, []);
  ...
  const openModal = (flightId, flightIndex) => {
  ...
  const transitions = useTransition(readyForAnimation ? fluege : [], {
    keys: flug => flug._id,
  ...
  const getRandomImage = (index, prefix = 'Plane') => {
    if (images.length > 0) {
      ...
      <section className="py-12 bg-white sm:py-16 lg:py-20">
        <div className="px-4 mx-auto sm:px-6 lg:px-8 max-w-7xl">
          <div className="max-w-6xl mx-auto">
            <div>
              <h1 className="text-2xl font-bold text-gray-900 sm:text-3xl">Flüge</h1>
              <p className="mt-2 text-sm font-normal text-gray-600">Discover amazing flights and book your trip</p>
            </div>
            {loading ? (
              ...
              <ul className="mt-8 space-y-5 lg:mt-12 sm:space-y-6 lg:space-y-10">
                {transitions((styles, flug, t, index) => (
                  <animated.li style={styles} key={flug._id} className="relative overflow-hidden bg-white border border-gray-200 rounded-md">
                    <div className="lg:flex bg-gray-100 p-4">
                      <div className="flex-shrink-0">
                        <Image
                          src={getRandomImage(index)}
                          alt={flug.Fluggesellschaft}
                          className="object-cover w-full h-48 lg:w-64 rounded-md"
                          layout="fixed"
                          width={256}
                          height={192}
                          quality={20}
                      ...
                      <div className="flex-1 px-4 py-6 sm:p-6 lg:p-8">
                        <div className="sm:flex sm:justify-between sm:items-start">
                          <div>
                            <h2 className="text-xl font-bold text-gray-900">{flug.Fluggesellschaft}</h2>
                            <p className="mt-1 text-sm text-gray-500">{flug.Abflugsort} - {flug.Zielort}</p>
                          </div>
                          <div className="mt-4 sm:mt-0">
                            <p className="text-sm font-medium text-gray-600">${flug.Ticketpreis} total</p>
                          </div>
                        </div>
                        <p className="mt-4 text-gray-600">{flug.Flugzeugtyp}</p>
                      </div>
                    </div>
                    <div className="px-4 py-6 sm:px-6 lg:px-4 bg-gray-100 border-t-2 border-gray-300">
                      {reviews[flug._id] && reviews[flug._id].slice(0, 2).map((review, reviewIndex) => (
                      ...
                      <button
                        onClick={() => openModal(flug._id, index)}
